...
│       ├── sui_tang/      ← 隋唐五代（90）
│       ├── song_liao_jin/ ← 宋辽金（113）
│       ├── yuan_ming_qing/← 元明清（106）
│       └── modern/        ← 近现代（0）
...
数据来源于 Marvis 碑帖释文数据集（per_stele 直读模式，482 件）及 Deep Research 补充校勘（12 件：睡虎地秦简、里耶秦简、中山王厝鼎、郭店楚简老子甲、李建中土母帖、林逋自书诗卷、鲜于枢道德经卷、王铎赠张抱一诗卷、邓石如篆书千字文、赵之谦篆书铙歌册、伊秉绶遂性草堂、吴昌硕临散氏盘），按本仓库 schema 整理。新增 12 件释文均经 2–3 个权威来源交叉校勘（详见 `释文/新增碑帖释文校勘报告.md`）。
